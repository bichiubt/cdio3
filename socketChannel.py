...
# Code này giúp chatbot Rasa giao tiếp với người dùng qua WebSocket (Socket.IO) thay vì API HTTP truyền thống.
#  Tối ưu cho chatbot real-time, hỗ trợ gửi hình ảnh, nút bấm, và tin nhắn dạng JSON.


logger = logging.getLogger(__name__)

# Thiết lập WebSocket (SocketBlueprint) để Rasa có thể giao tiếp với người dùng.
...
# Lớp SocketIOOutput giúp gửi tin nhắn từ chatbot đến WebSocket.
...
# Lớp SocketIOInput giúp nhận tin nhắn từ WebSocket và gửi đến Rasa xử lý.
...
# Xử lý kết nối (connect), ngắt kết nối (disconnect), và session (session_request).
        ...
        async def session_request(sid: Text, data: Optional[Dict]):
            ...
            logger.debug(f"User {sid} connected to socketIO endpoint.")
# Nhận tin nhắn từ người dùng (handle_message) và gửi phản hồi.
